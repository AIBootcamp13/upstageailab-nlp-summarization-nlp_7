...
# NLLB 모델 로드
MODEL_NAME = "ryusangwon/ko_en_nllb-200-distilled-600M-test"
...
def backtranslation(df, range_start=0, range_end=None, batch_size=8):
    ...
    for dialogue in tqdm(df['dialogue'][range_start:range_end], desc="Backtranslation"):
        ...
        try:
            en_batch = nllb_batch_translate(texts, src_lang="kor_Hang", tgt_lang="eng_Latn", batch_size=batch_size)
...
if __name__ == "__main__":
    ...
    range_start = 10000
    range_end = "end"
    df = df.iloc[range_start:]
    df = backtranslation(df, batch_size=8)
    df.to_csv(os.path.join(ROOT_DIR, "data", f"train_backtranslated_sentence_{range_start}_{range_end}.csv"), index=False)
